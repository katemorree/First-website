/* =========================================================
   ...
   ---------------------------------------------------------
   The KEY is the English text, exactly as it is written in
   the components. Look up a string with the <T> component or
   the useT() hook in lib/language.jsx:

       <T>Order Now</T>

   That means:
   - English needs no entries here; it is the source.
   - To change English wording, change it in the component AND
     change the key here, or the translation stops matching.
   - A missing key falls back to English rather than showing
     an empty element.
   - A few keys contain <br>, where a heading is split over two
     lines. <T> renders those as real line breaks. Word order
     differs between the three languages, so each of those has
     to be translated as a whole line rather than in pieces.
